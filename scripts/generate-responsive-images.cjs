...
const { log, warn, error, info, divider, elapsed, bytes, summary, c, B, D, GRN, YLW, CYN, R } = require('./_logger.cjs');
...
const JOBS = [
  ...
  // ── Avatars (56 + @2x 112) ─────────────────────────────────────────────────
  ...['viorel-cozma', 'oleg-groian', 'laura-turcan', 'andrei-aparatu', 'vlad-milea', 'elena-anghel']
    .map((name) => ({
      input: `${name}.webp`,
      outputs: [
        { suffix: 'avatar-56',  resize: { width:  56, height:  56, fit: 'cover', position: 'attention' }, quality: 84 },
        { suffix: 'avatar-112', resize: { width: 112, height: 112, fit: 'cover', position: 'attention' }, quality: 84 },
      ],
    })),
];
...
async function run() {
  ...
  for (const job of JOBS) {
    ...
    for (const out of job.outputs) {
      ...
      tasks.push(async () => {
        ...
        console.log(`  ${c(GRN, '+')}  ${c(D, rel)}  ${c(CYN, bytes(size))}  ${c(D, elapsed(tFile))}`);
      });
    }
  }
  ...
  info(`Generating ${c(B, String(tasks.length))} image(s)  ·  concurrency ${c(B, String(Math.min(CONCURRENCY, tasks.length)))}`);
  if (skippedN > 0) info(`Skipping ${skippedN} up-to-date output(s) (use --force to override)`);
...
run().catch((e) => { error(e.message || String(e)); process.exit(1); });
