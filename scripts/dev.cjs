...
function launch(descriptor, getSpawnArgs, restartOnCrash = true) {
  ...
  const start = () => {
    ...
    child.on("exit", (code, signal) => {
      ...
      warn(
        `[${name}] crashed (exit ${code ?? signal}), restarting in ${c(B, `${delay}ms`)}… (attempt ${crashCount})`,
      );
...
info(
  `Starting ${c(B, "CSS watcher")} + ${c(B, "dev server")} on ${c(GRN, `http://127.0.0.1:${PORT}`)}`,
);
