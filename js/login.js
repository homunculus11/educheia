...
const isAuthPath = (path) =>
  /\/(?:(?:src\/)?(?:login|register)(?:\.html)?)$/i.test(
    String(path || "").replace(/\/+$/, ""),
  );
...
const isSafeRedirect = (target) => {
  ...
  try {
    ...
    return (
      parsed.origin === window.location.origin && !isAuthPath(parsed.pathname)
    );
  } catch {
    return false;
  }
...
const persistUsernameAlias = (username, email) => {
  ...
  try {
    const raw = localStorage.getItem(USERNAME_EMAIL_MAP_KEY);
    const parsed = raw ? JSON.parse(raw) : {};
    parsed[String(username).trim().toLowerCase()] = String(email)
      .trim()
      .toLowerCase();
...
const shouldFallbackToRedirect = (err) => {
  const code = err?.code || "";
  return (
    code === "auth/popup-blocked" || code === "auth/web-storage-unsupported"
  );
};
...
if (loginForm) {
  loginForm.addEventListener("submit", async (e) => {
    ...
    if (!email) {
      alert(
        "Folosește adresa de email sau un username deja înregistrat pe acest browser.",
      );
      return;
    }
...
if (googleLoginButton) {
  googleLoginButton.addEventListener("click", async () => {
    if (window.location.protocol === "file:") {
      alert(
        "Google Sign-In nu funcționează din file://. Rulează aplicația pe localhost.",
      );
      return;
    }
