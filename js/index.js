...
const renderQuoteCarousel = (items) => {
  ...
  const fallbackSlides = [
    {
      quote: "„Educheia m-a făcut să-mi schimb cursul profesional în 6 luni.”",
      author: "— Ana, profesor de biologie",
    },
    {
      quote:
        "„În fiecare episod găsesc o întrebare pe care nu îndrăzneam s-o pun.”",
      author: "— Vlad, student",
    },
    {
      quote: "„Un spațiu rar în care educația e tratată cu curaj și empatie.”",
      author: "— Maria, mentor",
    },
  ];
...
const getLatestEpisode = (items) => {
  ...
  return (
    items.find((item) => {
      const title = item?.snippet?.title;
      return (
        typeof title === "string" && title.trim() && !/trailer/i.test(title)
      );
    }) || null
  );
};
...
const renderLatestEpisodeCard = (items) => {
  ...
  const publishedAt =
    latestEpisode?.snippet?.publishedAt ||
    latestEpisode?.contentDetails?.videoPublishedAt;
...
getEpisodes()
  .then((data) => {
    ...
    if (latestForPreview) {
      const vid =
        latestForPreview.contentDetails?.videoId ||
      ...
      if (vid) {
        ...
        if (titleEl)
          titleEl.textContent =
            extractEpisodeTopic(latestForPreview.snippet?.title) ||
            latestForPreview.snippet?.title ||
            "Episod nou";
      }
    }
  })
...
const setupReactiveSheen = () => {
  ...
  const writeCssVars = (motionX, motionY, speed) => {
    ...
    root.style.setProperty(
      "--hover-angle",
      `${Number.isFinite(angle) ? angle.toFixed(2) : "0"}deg`,
    );
  ...
  const animate = (time) => {
    const delta = Math.min(
      Math.max((time - previousFrameTime) / 16.67, 0.6),
      2.2,
    );
    ...
    const settled =
      Math.abs(pointer.x - core.x) < 0.15 &&
...
const updatePreviewUi = () => {
  ...
  // Play/pause icons
  document
    .getElementById("preview-icon-play")
    ?.classList.toggle("hidden", isPlaying);
  document
    .getElementById("preview-icon-pause")
    ?.classList.toggle("hidden", !isPlaying);
...
const setupPreviewPlayer = () => {
  // Close button
  document
    .getElementById("preview-player-close")
    ?.addEventListener("click", closePreviewPlayer);
  ...
  // Play/Pause toggle
  document
    .getElementById("preview-play-pause")
...
(() => {
  ...
  if (episodeCards.length && !prefersReducedMotion) {
    ...
    const episodeObs = new IntersectionObserver(
      (entries) => {
  ...
  if (guestsGrid && !prefersReducedMotion) {
    const revealGuests = () => {
      const cards = guestsGrid.querySelectorAll(
        ".guest-card:not(.guest-card-skeleton)",
      );
  ...
  if (listenCards.length && !prefersReducedMotion) {
    ...
    const listenObs = new IntersectionObserver(
      (entries) => {
        for (const entry of entries) {
          if (entry.isIntersecting) {
  ...
  if (!prefersReducedMotion && hasFinePointer) {
    const tiltTargets = document.querySelectorAll(
      ".episode-card, .listen-card, .metric-card",
    );
